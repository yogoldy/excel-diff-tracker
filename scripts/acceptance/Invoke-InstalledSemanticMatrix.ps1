...
$workbookPath = Join-Path $fixtures 'Installed Semantic Matrix.xlsx'
...
function Capture-AppEvidence {
    ...
    $script:mainWindow = Find-UiaWindow -Title 'Excel Diff Tracker' -TimeoutSeconds 10
    $screenshotPath = Join-Path $screenshots ('sequence-{0:D2}.png' -f $Sequence)
    $uiaPath = Join-Path $uia ('sequence-{0:D2}.json' -f $Sequence)
...
function Complete-MatrixPhase {
    ...
    for ($index = 0; $index -lt $Checks.Count; $index++) {
        $probeName = 'sequence-{0:D2}-check-{1:D2}.json' -f $Sequence, ($index + 1)
    ...
    $sourceHash = (Get-FileHash $workbookPath -Algorithm SHA256).Hash.ToUpperInvariant()
    ...
    $workbookCopy = Join-Path $workbookStates ('sequence-{0:D2}.xlsx' -f $Sequence)
    Copy-Item -LiteralPath $workbookPath -Destination $workbookCopy
    ...
    $report = $null
    if (-not $Baseline) {
        $reportCopy = Join-Path $reports ('sequence-{0:D2}.md' -f $Sequence)
...
try {
    ...
    $matrixSheet = $workbook.Worksheets.Item(1)
    ...
    Add-TrackedWorkbook -Path $workbookPath
    Complete-MatrixPhase -Sequence 0 -Name 'silent baseline' -Action 'register deterministic workbook through the installed app' -CellChangeCount 0 -SheetChangeCount 0 -Checks @(
        [pscustomobject]@{ arguments = @() }
    ) -ActionStartedUtc $startedUtc -CtrlSaveUtc $null -Baseline
    ...
    [System.Windows.Forms.SendKeys]::SendWait('{ENTER}')
    $savedUtc = Save-ExcelByKeyboard
    Complete-MatrixPhase -Sequence 7 -Name 'sheet add' -Action 'insert a worksheet and name it Matrix Added' -CellChangeCount 0 -SheetChangeCount 1 -Checks @(
    ...
    Start-Sleep -Milliseconds 300
    $savedUtc = Save-ExcelByKeyboard
    Complete-MatrixPhase -Sequence 12 -Name 'sheet remove' -Action 'activate and delete Matrix Renamed from the Home ribbon' -CellChangeCount 0 -SheetChangeCount 1 -Checks @(
    ...
    Assert-Matrix 'baseline and twelve saves have thirteen distinct stable hashes' ($distinctHashes.Count -eq 13) "distinct=$($distinctHashes.Count)"
}
catch {
    $failed = $true
    ...
    foreach ($value in @($matrixSheet, $workbook, $excel)) {
    ...
    $result = [ordered]@{
        ...
        status = if (-not $failed -and $phases.Count -eq 13 -and @($assertions | Where-Object { -not $_.passed }).Count -eq 0) { 'Passed' } else { 'Failed' }
        ...
        workbook = [ordered]@{
            path = $workbookPath
            finalSequence = if ($phases.Count -gt 0) { $phases[$phases.Count - 1].sequence } else { $null }
        }
        phases = $phases
...
if ($failed -or $phases.Count -ne 13 -or $result.status -ne 'Passed' -or @($assertions | Where-Object { -not $_.passed }).Count -ne 0) {
